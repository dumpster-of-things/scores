...
for depends in 'op' 'wcl' 'iterdate' ; do [[ ! -x $(which $depends) ]] || { mkdir -p ${Od%\/*}/scripts ; curl -s "https://raw.githubusercontent.com/dumpster-of-things/scripts/main/$depends" -o ${Od%\/*}/scripts/$depends ; chmod 755 ${Od%\/*}/scripts/$depends ; alias $depends=${Od%\/*}/scripts/$depends ; } ; done
